...
@section('header')
    ...
    @if(auth('admin')->user()->institutional_id == 3)
        ...
        @if(date("m") > "03")
            <script type="text/javascript">
                var visitor_mar = <?php echo \App\Model\visitor_counting::visitorLineChartMar($institutional_id); ?>;
                google.charts.load('current', {'packages':['corechart']});
                google.charts.setOnLoadCallback(drawChart);
                function drawChart() {
                    var data_mar = google.visualization.arrayToDataTable(visitor_mar);
                    var options_mar = {
                        title: "@lang('messages_be.dashboard_visitor_title')",
                        curveType: 'function',
                        legend: { position: 'bottom' }
                    };
                    var chart_mar = new google.visualization.LineChart(document.getElementById('linechart-mar'));
                    chart_mar.draw(data_mar, options_mar);
                }
            </script>
        @endif
    @endif
...
<div class="container-fluid">
    ...
    @if(auth('admin')->user()->institutional_id == 3)
        ...
        @if(date("m") > "03")
            <div class="row">
                ...
                <!-- Area Chart -->
                @if(\App\Model\visitor_counting::visitorLineChartMar($institutional_id) != '[["Date","Visit"]]')
                    <div class="col-xl-12 col-lg-12">
                        <div class="card shadow mb-4">
                            <!-- Card Header - Dropdown -->
                            <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    <div id="linechart-mar" style="width: 100%; height: 320px;"></div>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <div class="row">
        ...
        <div class="col-xl-12 col-lg-12">
            <div class="card shadow mb-4">
                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                    <h6 class="m-0 font-weight-bold text-primary">@lang('messages_be.dashboard_visitor_filter_priode')</h6>
                </div>
                <div class="card-body">
                    <form method="get" action="{{route('dashboard-filter-grafik')}}">
                        <input type="hidden" name="institutional_id" value="{{$institutional_id}}">
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label>@lang('messages_be.info_from')</label>
                                    <input type="date" name="from" class="form-control">
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label>@lang('messages_be.info_to')</label>
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <button class="btn btn-block btn-primary">@lang('messages_be.info_btn_search')</button>
                            </div>
                        </div>
                    </form>
                </div>
